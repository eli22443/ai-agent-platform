...
**Concepts.** Source-code chunking that respects structure, embedding models and dimensionality, vector upsert and query, metadata filtering, namespace design per task workspace, hybrid retrieval strategy.
...
**Metadata per vector.** Task identifier, optional repository identifier, file path, language, chunk start and end lines, and optional commit SHA for provenance (not used for index skip/rebuild).
